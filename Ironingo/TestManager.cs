...
namespace Ironingo
{
    public class TestManager
    {
        ...
        private int currentIndex = -1;//чтобы при первом вызове GetNextWord() увеличить на 1 и получить индекс 0 
        ...
        public Word GetNextWord()//метод для получения след.слова из списка
        {
            currentIndex++;
            if (currentIndex >= words.Count)
                return null; // Конец теста
        ...
        public void CheckAnswer(string userAnswer)//метод для проверки ответа пользователя
        {
            ...
            //Сравнивает ответ пользователя (userAnswer) с правильным переводом текущего слова
            //userAnswer?.Trim() убирает лишние пробелы и проверяет на пустоту
            //Сравнение без учёта регистра из за OrinalIgnareCase
